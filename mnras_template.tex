...
\end{align}
\noindent is the Lam\'e coefficient. The first two jump conditions can be combined, allowing us to ignore the scaling of $U$ and $V$ in the crust and core. This gives us %mention how dividing one _cr by another _cr removes the scaling, so as long as we have f(X1_cr/X2_cr)=g(Y1_co/Y2_co), we can ignore scaling.
\begin{align}
...
\tilde{U}=\frac{d\ln\left(M_r\right)}{d\ln\left(r\right)}=4\pi r^2\rho\frac{r}{M_r},\;\;\;c_1=\left(\frac{r}{R_*}\right)^3\frac{M_*}{M_r},\;\;\;\Omega^2=\frac{\omega^2R_*^3}{GM_*},
\end{align}
\noindent The condition at the centre follows from the requirement that $U_{\rm co}$ and $V_{\rm co}$ be regular at $r=0$: %Check that U and V being regular is correct (y1 and y2 must be regular, does this mean U and V must be too?)
\begin{align}
...
\label{fig:trace_minima}  %need to change y axis direction
\end{figure}
