...
//function to smooth the input array, hopefully making cubic splines more reasonable
...
//Uses the equation for dm/dr from the TOV equations
...
//Uses the equation for dp/dr from the TOV equations
...
//shear modulus from Strohmayer 1991 (unused, mu provided in EoS files)
double mu_val(double r){
  double output,bary_dens_val;
  bary_dens_val=spline_value(rsteps,bary_dens,r_pos,d_bary_dens,r);
  if (bary_dens_val>NB_CC){return 0.0;}  //if in the core, shear is zero
...
//derivative of logp(logrho) (unused, far more acurate adiabatic index provided in EoS files)
void setup_g1(double g1[],int lines,double rho_arr[],double pres_arr[],double EoS_bary_dens[]){
  int i;
  double j=0.5;  //0.5 to 1.0 seems okay-ish, but the results are sigificantly dependant on this choice!
