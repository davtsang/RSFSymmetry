...
void main(int argc, char *argv[])
{
  ...
  FILE *in_file=fopen("J_L_K_f_2.txt","r"); 
  ...
  for(i=0;i<lines;i++){
    ...
    fscanf(in_file,"%le  %le  %le  %le",&J[i],&L[i],&K[i],&f[i]);                                      //J vs L contours
//    fscanf(in_file,"%le  %le  %le  %le",&K[i],&J[i],&L[i],&f[i]);                                    //L vs K contours  (Also need to check output is on same order!)
//    fscanf(in_file,"%le  %le  %le  %le",&J[i],&K[i],&L[i],&f[i]);                                    //J vs K contours
  }


//optional bit of code to sort the input file by J,L,K (in that order of priority);
/*
  FILE *out_file=fopen("J_L_K_f_2.txt","w"); 
  double Ksymmin=1e+10,Jval=25.0,Lval,Lvalmin,Lvalmax;
  int line_Kmin;
  do {
    Lvalmin=1e+10;
    Lvalmax=-1e+10;
    for(i=0;i<lines;i++){
      if (Lvalmin>L[i] && J[i]==Jval){Lvalmin=L[i];}
      if (Lvalmax<L[i] && J[i]==Jval){Lvalmax=L[i];}
    }
    Lval=Lvalmin;
    do {
      do {
        Ksymmin=1e+10;
        for(i=0;i<lines;i++){
          if (J[i]==Jval && L[i]==Lval && f[i]>0){
            if (Ksymmin>K[i]){Ksymmin=K[i];line_Kmin=i;}
          }
        }
        if (Ksymmin<1e+9){
          fprintf(out_file,"%2.1lf  %2.2lf  %4.2lf  %9.6lf\n",J[line_Kmin],L[line_Kmin],K[line_Kmin],f[line_Kmin]);
          f[line_Kmin]=-1000;
        } else {
          break;
        }
...
void contour(double f_target, double Kval, int lines){
  ...
  //find the J,L grid in the input data
  Jvalues[0]=J[0];
  Lvalues[0]=L[0];
  ...
  for (i=0;i<Jvals;i++){
    ...
    find_JL(lines_J[i],f_J_Kval[i],L_J_Kval[i],f_target,Jvalues[i],Kval);    //find the L value the target frequency occurs at for each J value in the input grid (for K=Kval)
  }
  ...
  for (i=0;i<Lvals;i++){
    ...
    find_LJ(lines_L[i],f_L_Kval[i],J_L_Kval[i],f_target,Lvalues[i],Kval);    //find the J value the target frequency occurs at for each L value in the input grid (for K=Kval)
  }


  //sort the J,L values that the target frequency occurs at by L value (lowest L value first)
...
//  }printf("\n");
  ...
  //if cubic splines are worth using, use them to interpolate the J,L values that give the target frequency
  if (ffound>200000){  //set to >2 to use cubic splines, set to a large value to always use linear interpolation
    for (i=0;i<ffound;i++){
      J_ffound[i]=J_ffound[i]+i/1e+4;  //make tiny adjustments to each J value to make sure splines don't become linear due to two J values being equal
    }
  
    d_J_ffound=malloc(ffound*sizeof(double));
    cubic_spline(ffound,J_ffound,L_ffound,d_J_ffound);

    //get the maximum and minimum L values that the target frequency has been found at
    for(i=0;i<ffound;i++){
    ...
    do {
      printf("%le  %le  %le\n",spline_value(ffound,J_ffound,L_ffound,d_J_ffound,testL),testL,Kval);//!!!switch these outputs around to be in the same order as inputs when doing J,K or L,K!!!
      testL=testL+0.1;
    } while (testL<L_max);
  }  
  ...
  //else print out the J,L,K=Kval values without any interpolation
    ...
    for (i=0;i<ffound;i++){
      printf("%le  %le  %le\n",J_ffound[i],L_ffound[i],K_ffound[i]);//!!!switch these outputs around to be in the same order as inputs when doing J,K or L,K!!!
    }
  }
...
// function for finding the J,L values that give the target frequency (for K=Kval) by interpolating between L values for each J value
void find_JL(int lines, double f_J_K[], double L_J_K[], double f_target, double Jval, double Kval){
  ...
  if (lines>1){
    //setup cubic splines for f vs L with constant J,K
    cubic_spline(lines,f_J_K,L_J_K,d_f_J_K);

    //loop over L values, interpolating to get the frequency
    Lval=L_J_K[0];
    f_val=spline_value(lines,f_J_K,L_J_K,d_f_J_K,Lval);
    do{
      f_val_next=spline_value(lines,f_J_K,L_J_K,d_f_J_K,Lval+d_Lval);
      //printf("%le  %le  %le\n",Jval,Lval,f_val);
      if ((f_val>f_target && f_val_next<f_target) || (f_val<f_target && f_val_next>f_target)) {  //if the target frequency is between this L and L+L_step...
...
// function for finding the J,L values that give the target frequency (for K=Kval) by interpolating between J values for each L value
void find_LJ(int lines, double f_L_K[], double J_L_K[], double f_target, double Lval, double Kval){
  ...
  if (lines>1){
    //setup cubic splines for f vs J with constant L,K
    d_f_L_K=realloc(d_f_L_K,lines*sizeof(double));
    cubic_spline(lines,f_L_K,J_L_K,d_f_L_K);

    //loop over J values, interpolating to get the frequency
    Jval=J_L_K[0];
