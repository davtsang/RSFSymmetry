...
void main(int argc, char *argv[])
{
  ...
  FILE *in_file=fopen("J_L_K_f_M160.txt","r"); 
  ...
  //read input files
  for(i=0;i<lines;i++){                                      //change to K,J,L (J,K,L) to get L vs K (J vs K) contours
    //fscanf(in_file,"%le  %le  %le  %le  %le  %le  %le  %le  %le  %le",&J[i],&L[i],&K[i],&M,&f[i],&t1,&t2,&t3,&t4,&t5);
    fscanf(in_file,"%le  %le  %le  %le",&J[i],&L[i],&K[i],&f[i]);
//    fscanf(in_file,"%le  %le  %le  %le",&K[i],&J[i],&L[i],&f[i]);
//    fscanf(in_file,"%le  %le  %le  %le",&J[i],&K[i],&L[i],&f[i]);
  }
...
//  }printf("\n");
  ...
  if (ffound>200000){  //set to >2 to use splines, set to a large value to always use linear interpolation
    for (i=0;i<ffound;i++){
