...
//modified to use rdy/dr instead of (1+V)dy/dz (they are equal, and I find rdy/dr easier to use)
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
#include "nr_codes.h"
...
void derivatives_crust(double rpos,double var_in[],double d_var_out[]){
  ...
  d_var_out[0]=(1.0/rval)*(-(1.0+2.0*(a2/a3))*var_in[0]+(1.0/a3)*var_in[1]+var_in[5]*(var_in[5]+1.0)*(a2/a3)*var_in[2]);
  d_var_out[1]=(1.0/rval)*((-c1val*Vtildeval*var_in[4]-4.0*Vtildeval+Utildeval*Vtildeval+12.0*g1val*a1/a3)*var_in[0]+(Vtildeval-4.0*a1/a3)*var_in[1]+var_in[5]*(var_in[5]+1.0)*(Vtildeval-6.0*g1val*a1/a3)*var_in[2]+var_in[5]*(var_in[5]+1.0)*var_in[3]);
  if (a1 != 0.0){
    d_var_out[2]=(1.0/rval)*(-var_in[0]+(1.0/a1)*var_in[3]);
  } else {                                   //avoid any risk of divide by zero
    d_var_out[2]=(1.0/rval)*(-var_in[0]);    //if a1=0.0, mu=0.0 --> must be in core --> z4=0.0
  }
  d_var_out[3]=(1.0/rval)*((Vtildeval-6.0*g1val*a1/a3)*var_in[0]-(a2/a3)*var_in[1]+(-c1val*Vtildeval*var_in[4]+(2.0/a3)*((2.0*var_in[5]*(var_in[5]+1.0)-1.0)*a1*a2+2.0*(var_in[5]*(var_in[5]+1.0)-1.0)*a1*a1))*var_in[2]+(Vtildeval-3.0)*var_in[3]);
  d_var_out[4]=0.0;  //don't modify the frequency or l
  d_var_out[5]=0.0;
}
...
void derivatives(double rpos,double var_in[],double d_var_out[]){
  ...
  d_var_out[0]=(1.0/rval)*((Vtildeval/g1val-3.0)*var_in[0]+((var_in[3]*(var_in[3]+1)/(c1val*var_in[2]))-Vtildeval/g1val)*var_in[1]);
  d_var_out[1]=(1.0/rval)*((c1val*var_in[2]+Arval)*var_in[0]+(1.0-Utildeval-Arval)*var_in[1]);
  d_var_out[2]=0.0;  //don't modify the frequency or l
  d_var_out[3]=0.0;
}

void core(double *z1,double *y1,double *y2,double omega,int l,int print,double sigma){
  int i,small_step;
  double dr,c1omega,omega2,drused,drnext,rpos,r,scale,Vtilde_ss,Arval=0.0,Utilde_ss;
  double variables[4];
  for (i=0;i<rsteps;i++){
  ...
  if (print){
    scale=1.0/(*y1);                                                          //if writing U and V/10 out, find scale for y1 so that U(Rcc)=1
  }
  ...
  *y2=(*y2)*(*y1);
  ...
  rpos=r_pos[small_step];
  drnext=(r_pos[small_step+1]-r_pos[small_step])*10.0;
  drused=drnext;
  do {
    if (rpos+drnext>boundarypos){drnext=boundarypos-rpos;}
    RK_var_step(variables,4,&rpos,drnext,TOL,&drused,&drnext,derivatives);    //take RK4 step
...
void crust(double *z1,double *z2,double *z3,double *z4,double omega,int l,double sigma,int print,double *scale){
  int i,small_step;
  double dr,drused,drnext,rpos,r,c1omega,omega2,Vtilde_ss,Utilde_ss;
  double variables[6];
  ...
  *z4=0.0;
  ...
  do {
    if (rpos+drnext<boundarypos){drnext=boundarypos-rpos;}
    RK_var_step(variables,6,&rpos,drnext,TOL,&drused,&drnext,derivatives_crust);    //take RK4 step
...
//  if (print)printf("crust scale=  %le\n",(*scale));
  if (!print){(*scale)=1.0/variables[0];}
}


//function for testing the boundary condition, and determine the next test frequency
