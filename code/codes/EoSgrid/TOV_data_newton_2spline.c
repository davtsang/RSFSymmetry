...
//PRINT OUT THE CUBIC SPLINE INTERPOLATION OF THE EoS
/*
  p=pres_arr[0];
  dp=pres_arr[0]/10.0;
...
/*
  char TOVname[80]="",strJ[2]="",strL[2]="",strK[2]="";double rcc;
  strcat(TOVname,"TOV_MRcc/TOVcc_");
  ...
  strcat(TOVname,strL);
  strcat(TOVname,"_");
  sprintf(strK,"%d",(int)(atof(argv[3])));
  strcat(TOVname,strK);
  strcat(TOVname,".txt");
  ...
  do{
    ...
    do {
      ...
      //output some data every j RK4 steps
      rho=spline_value(ARRAY_SIZE,rho_arr,pres_arr,d_rho,p);
      bary=spline_value(ARRAY_SIZE,bary_arr,pres_arr,d_bary,p);
      if (bary <= nbcc){rcc=r;break;}
...
//      printf("%.18le  %.18le  %.18le\n",rho,r/100000,m*1.47667/1.99e33);
      p_old=p;
  
    } while (p > pres_arr[1]);  //leave loop if P is below the boundary (what is the best boundary?)
  
    rho=spline_value(ARRAY_SIZE,rho_arr,pres_arr,d_rho,p_c);
    fprintf(dens_output,"%.15le  %.15le\n",r,rho);  //print out extra line just to make sure total radius is written down somewhere
//    printf("neutron star of mass %lf M_sun and radius %lf Km, calcualted using central pressure %.4e and central density %.4e.\n",m/(1.989E+33),r/(1.0E+05),p_c,rho);


    fprintf(fp,"%lf   %lf\n",rcc/(1.0E+05),m/(1.989E+33));
    p_c=p_c+dp_c;
    if (p_c > dp_c*100.0){dp_c=dp_c*10.0;}
  }while (p_c<pres_arr[ARRAY_SIZE-1]);
