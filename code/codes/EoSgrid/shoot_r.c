...
//this file is not currently used, it is just a backup of the version that uses d/dr (see shoot.c for the version currently being used)
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
#include "nr_codes.h"
...
void derivatives_crust(double rpos,double var_in[],double d_var_out[]){
  ...
  if (a1 != 0.0){
    ...
    d_var_out[2]=(1.0/rval)*(-var_in[0]);  //if a1=0.0, shear_mod=0.0 --> if omega is correct then z4=0.0
  }
...
//  if (print)printf("crust scale=  %le\n",(*scale));
  if (!print){(*scale)=1.0/(variables[0]*rpos/boundarypos);}
}
