...
void main(int argc, char *argv[])
{
  ...
  int i, lines;
  char c;
  double m1,m2,f_range,M,temp;

  if (argc != 3) {printf("Input: ./find_f [frequency (Hz)] [Ksym (MeV)]\n");exit(0);}

  f_target=atof(argv[1]);
  Kval=atof(argv[2]);

  FILE *in_file=fopen("all_ctav.txt","r"); 
...
//    fscanf(in_file,"%le  %le  %le  %le  %le",&J[i],&L[i],&K[i],&f[i],&temp);
    fscanf(in_file,"%le  %le  %le  %le  %le",&K[i],&J[i],&L[i],&f[i],&temp);          //f is actually average ct, temp is freq.
//    fscanf(in_file,"%le  %le  %le  %le  %le",&J[i],&K[i],&L[i],&f[i],&temp);
  }
...
//  }printf("\n");
  ...
  if (ffound>200000){  
    ...
    do {
      printf("%le  %le  %le\n",Kval,spline_value(ffound,J_ffound,L_ffound,d_J_ffound,testL),testL);//switch these outputs around to be in the same order as inputs when doing J,K or L,K
      testL=testL+0.1;
    } while (testL<L_max);
  }  
  ...
  //else print out the J,L,K=Kval values without any interpolation
    ...
    for (i=0;i<ffound;i++){
      printf("%le  %le  %le\n",K_ffound[i],J_ffound[i],L_ffound[i]);//switch these outputs around to be in the same order as inputs when doing J,K or L,K
    }
  }
//  printf("\n");
