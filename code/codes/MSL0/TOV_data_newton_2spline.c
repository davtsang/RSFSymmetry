...
void main (int argc, char *argv[]) {
  ...
  if (argc != 5) {printf("Input: ./star [J (i.x)] [L (j.00)] [K (k.00)] [M* (solar masses)]\n");exit(0);}
...
/*------------smooth eos data to avoid repeated lines, which cause errors in the cubic splines------------*/
...
/*-------------use seperate splines in the core and crust to avoid errors at the discontinuity-------------*/
...
/*-----------------loop over central pressure values until total mass is the chosen value-----------------*/
  p_c=1.0e+34;
  dp_c=1.0e+34;
  do{
    m=0.0;
...
/*---------------------------------------different optional outputs---------------------------------------*/
//PRINT OUT THE INTERPOLATION OF THE EoS
/*
  p=pres_arr[0];
  dp=pres_arr[0]/10.0;
  FILE *new=fopen("spline.txt","w");
  do{
    if (p>=pres_arr[ARRAY_SIZE-lines_core]) {
      rho=spline_value(lines_core,core_rho,pres_core,d_rho_core,p);
    } else if (p<=pres_arr[ARRAY_SIZE-lines_core-1]) {
...
//PRINT OUT THE M*(R*) GRAPH
/*
  char TOVname[80]="",strJ[2]="",strL[2]="";
  strcat(TOVname,"TOV_MR/TOV_");
  sprintf(strJ,"%d",(int)(atof(argv[1])));
  strcat(TOVname,strJ);
  strcat(TOVname,"_");
  sprintf(strL,"%d",(int)(atof(argv[2])));
  strcat(TOVname,strL);
  strcat(TOVname,".txt");

  p_c=1.0e+32;
  dp_c=1.0e+31;
  FILE *fp=fopen(TOVname,"w");
  do{
    m=0.0;
    r=0.0;
    p=p_c;
    ...
    do {
      ...
      p=variables[0];
      m=variables[1];
    } while (p > pres_arr[1]);  //leave loop if pressure is below the range in the EoS file

    fprintf(fp,"%lf   %lf\n",r/(1.0E+05),m/(1.989E+33));
    p_c=p_c+dp_c;
    if (p_c > dp_c*100.0){dp_c=dp_c*10.0;}
  }while (p_c<pres_arr[ARRAY_SIZE-1]);  //increase central pressure until it is above the range in the EoS file
...
/*----------------solve the TOV equations for 1.4 solar mass star, while printing out data----------------*/
  ...
  } while (p > pres_arr[1]);  //leave loop if p is outside the pressure range in the EoS
