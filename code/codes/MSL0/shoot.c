...
extern double *g1, *d_g1, *x, *d_rpos, *r_pos, *d_mass_r, *mass_r, *density, *d_density, *pressure, *d_pressure;
...
void derivatives_crust(double xpos,double var_in[],double d_var_out[]){
  ...
  if (a1 != 0.0){
    ...
    d_var_out[2]=(1.0/(1.0+Vtildeval))*(-var_in[0]);    //if a1=0.0, mu=0.0 --> must be in core --> z4=0.0
  }
...
void crust(double *z1,double *z2,double *z3,double *z4,double omega,int l,double sigma,int print,double *scale){
  ...
  if (print){
    *scale=1.0/(*z1);                                                         //if writing U and V out, find scale for z1 so that U(Rcc)=1
  }
  ...
  *z2=Vtilde_ss-c1omega-4.0+Utilde_ss;                                        //get z2 value from the surface boundary condition
...
//function for determining the next trial eigenvalues
