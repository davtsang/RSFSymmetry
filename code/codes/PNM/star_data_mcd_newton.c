...
/*----------get dimensionless properties of star and setup splines----------*/
  //printf("\n\n");
  //for (i=0;i<rsteps;i++){printf("%le  %le\n",r_pos[i]/100000.0,g1[i]);}exit(0);
/*--------------------use shooting method to find sigma---------------------*/
  ...
  if (option == 3){
    ...
    do{
      ...
      fprintf(z2_z1file,"%.12le  %.12le  %.12le  %.12le\n",omega,z2z1min,sigma/(2.0*PI),z2z1min*r_pos[rsteps-1]*scale_z2z1/boundarypos);// NOTE! NEW SCALES ADDED, SO IT CURRENTLY PRINTS OUT V(R*) ON THE XAXIS, NOT z3(R*)!
      fprintf(z4file,"%.12le  %.12le  %.12le  %.12le\n",omega,z4min,sigma/(2.0*PI),z4min*r_pos[rsteps-1]*scale_z4/boundarypos);
