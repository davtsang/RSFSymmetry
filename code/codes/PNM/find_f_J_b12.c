...
void main(int argc, char *argv[])
{
  ...
  for(i=0;i<lines;i++){                                      //change to L,K,J (J,K,L) to get L vs K (J vs K) contours
    fscanf(in_file,"%le  %le  %le  %le",&J[i],&K[i],&L[i],&f[i]); // J,a0,b12,f
  }
...
/*
  if (chirp>0){
    double tc_t=0.01,dtc_t=0.01;
    do{
      printf("%lf  %lf\n",tc_t,pow(800.0/1.41*tc_t*pow(chirp/1.2,5.0/3.0),-3.0/8.0)*1000.0);
      tc_t=tc_t+dtc_t;
      if (tc_t>=9.5*dtc_t) dtc_t=dtc_t*10.0;
    }while(tc_t<=100);
  }
*/
}
...
//  }printf("\n");
  ...
  //sort by J value (lowest J value first)
  i_sort=0;
  L_temp=realloc(L_temp,ffound*sizeof(double));
  J_temp=realloc(J_temp,ffound*sizeof(double));
  K_temp=realloc(K_temp,ffound*sizeof(double));
  ...
  if (ffound>20000){  //(((currently set to use linear splines, seems to be too difficult for cubic to manage)))
    for (i=0;i<ffound;i++){
    ...
    do {
      Jval=spline_value(ffound,J_ffound,L_ffound,d_J_ffound,testL);
      a0=Kval;
      b12=testL;
      Cl=-19.47*a0+1.56*b12-59.22;
      Ck=5*(Cl+50.22)+7.79*b12-258.3;
      printf("%le  %le  %le\n",Jval,6.7*Jval+Cl,18.4*Jval+Ck);
//      printf("%le  %le  %le\n",spline_value(ffound,J_ffound,L_ffound,d_J_ffound,testL),a0,b12);
      testL=testL+0.001;
    } while (testL<L_max);
  }  
...
void find_JL(int lines, double f_J_K[], double L_J_K[], double f_target, double Jval, double Kval){
  double Lval,f_val,f_val_next,d_Lval=0.001;
...
// function for finding the J,L,K=x values that give the target frequency by interpolating between L values for each J value
void find_LJ(int lines, double f_L_K[], double J_L_K[], double f_target, double Lval, double Kval){
  double Jval,f_val,f_val_next,d_Jval=0.001;
