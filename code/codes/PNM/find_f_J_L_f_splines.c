...
void main(int argc, char *argv[])
{
  ...
  FILE *in_file=fopen("J_L_f.txt","r"); 
  ...
  if (chirp>0){
    ...
    if (df_choice==1){
      contour(f_target+f_range,Kval,lines);
    } else if (df_choice==2){
      contour(f_target-f_range,Kval,lines);
    } else {
...
void find_JL(int lines, double f_J_K[], double L_J_K[], double f_target, double Jval, double Kval){
  double Lval,f_val,f_val_next,d_Lval=1.0;
  ...
  if (lines>1){
    ...
    Lval=20.0;
    do{
      if (Lval>=L_J_K[0] && Lval<=L_J_K[lines-1]){
...
// function for finding the J,L,K=x values that give the target frequency by interpolating between L values for each J value
void find_LJ(int lines, double f_L_K[], double J_L_K[], double f_target, double Lval, double Kval){
  double Jval,f_val,f_val_next,d_Jval=0.1;
  ...
  if (lines>1){
    ...
    Jval=26.0;
    do{
      if (Jval>=J_L_K[0] && Jval<=J_L_K[lines-1]){
